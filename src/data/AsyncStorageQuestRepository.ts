...
export class AsyncStorageQuestRepository implements IQuestRepository {
  async getQuests(): Promise<Quest[]> {
    try {
      ...
      // If empty, initialize with seed data for mockup purposes
      await this.saveQuests(seedQuests);
      return seedQuests;
    } catch (e) {
      console.error('Error reading quests from AsyncStorage', e);
      return [];
    }
  }
  ...
  private async saveQuests(quests: Quest[]): Promise<void> {
    try {
      ...
      console.error('Error saving quests to AsyncStorage', e);
    }
  }
